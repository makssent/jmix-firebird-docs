...
In this chapter, you will add the `picture` attribute to the `User` entity and learn how to upload and display images in the UI.
...
The dialog window that appears will show all views that display the `User` entity. Select the `User.detail` view and click *OK*.
...
Before running the application, create a Liquibase changelog for the new attribute.

Create the `070-add-user-picture.xml` file using the *Data Stores* -> *Main Data Store* -> *Liquibase: changelog.xml* section of the *Jmix* tool window, or in the `src/main/resources/com/company/onboarding/liquibase/changelog` directory in the project tree. It should add the `PICTURE` column to the `USER_` table.
...
This single changeset uses `addColumn` because the `USER_` table already exists and only the new column needs to be added.

The column type is `varchar(1024)`, matching the length in the `@Column` annotation. The image itself is not stored in the database: the column contains a `FileRef`, which is a string reference to a file in the file storage directory. No `nullable` constraint is specified because the picture is optional.
====
...
* {jmix-docs}/files/uploading-files.html[Uploaded files^] can be stored in a {jmix-docs}/files/file-storage.html[file storage^] and linked with entities using attributes of the `FileRef` type.

* The {jmix-docs}/flow-ui/vc/components/fileStorageUploadField.html[FileStorageUploadField^] component allows you to upload files, save them into the file storage and link to an entity attribute.

* The {jmix-docs}/flow-ui/vc/html-components/image.html[Image^] component can display images saved in the file storage.