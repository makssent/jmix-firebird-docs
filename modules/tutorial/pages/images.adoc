...
Создайте файл `070-add-user-picture.xml` - в разделе *Data Stores* -> *Main Data Store* -> *Liquibase: changelog.xml* окна инструментов *Jmix*, либо в каталоге `src/main/resources/com/company/onboarding/liquibase/changelog` в дереве проекта. Он должен добавлять в таблицу `USER_` столбец `PICTURE`.
...
Тип столбца - `varchar(1024)`, по длине из аннотации `@Column`. Само изображение в базе не хранится: в колонке лежит `FileRef` - строковая ссылка на файл в файловом хранилище, а хранилище представляет собой каталог на диске. Ограничение `nullable` не задано - изображение необязательно.
====
...
* {jmix-docs}/files/uploading-files.html[Загруженные файлы^] могут храниться в {jmix-docs}/files/file-storage.html[файловом хранилище^] и связываться с сущностями с использованием атрибутов типа `FileRef`.

* Компонент {jmix-docs}/flow-ui/vc/components/fileStorageUploadField.html[FileStorageUploadField^] позволяет загружать файлы, сохранять их в хранилище файлов и связывать с атрибутом сущности.

* Компонент {jmix-docs}/flow-ui/vc/html-components/image.html[Image^] может отображать изображения, сохраненные в файловом хранилище.