...
[WARNING]
====
Before starting the project, review the xref:firebird:limitations.adoc[Firebird limitations] and decide whether this DBMS is suitable for your task. Some Jmix and Studio features are unavailable with Firebird, including automatic Liquibase changelog generation.
====

This tutorial introduces you to the essentials of Jmix by walking you through building a simple full-stack web application. It covers the entire development process, from data modeling to creating UI and managing access rights.

Unlike the original Jmix tutorial, this tutorial uses Firebird as the database. The Jmix platform does not support Firebird out of the box; support is provided by a separate add-on. Along with the standard development steps, you will learn how to configure Firebird and how its behavior differs from databases supported by the platform out of the box.

To get the most out of this tutorial, you should be familiar with Java and relational databases.

[NOTE]
====
Instead of the tutorial, you can take our free https://www.udemy.com/course/rapid-application-development-with-jmix/[Full Stack Java Web Development^] course on Udemy.

Both the course and the tutorial cover the same product concepts. Note that the course uses the default HSQLDB database and is not designed for Firebird, so the connection setup and Liquibase changelog generation steps differ from those described here.
====

[[install]]
== Installation

Check system requirements and install Jmix Studio and Java Development Kit (JDK) as described in the https://docs.jmix.io/jmix/setup.html[Setup^] section of the Jmix documentation.

In addition to the standard installation, you will need:

* A *running Firebird server* and a user account with privileges to create tables.
* An *empty database* created in advance.
...
The complete application source code is available in the https://github.com/jmix-framework/jmix-onboarding[jmix-framework/jmix-onboarding^] GitHub repository. We recommend following instructions of this tutorial to create the application step by step, but if you want to just investigate the completed example, clone the repository and open it in Jmix Studio as described in the {jmix-docs}/studio/project.html#opening-existing-project[Opening Existing Project^] section.
...
* _HR Managers_ register new employees within their departments and start the onboarding process by generating steps for a particular user. They can also track the onboarding process of their department employees.
+
