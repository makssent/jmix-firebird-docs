...
[WARNING]
====
Скрипт Liquibase Studio не сгенерирует - его нужно написать вручную, как показано в разделе xref:simple-crud.adoc#run-app[Запуск приложения] главы про CRUD. Скрипт должен добавлять в таблицу `USER_` столбец `ONBOARDING_STATUS` типа `INT`, который соответствует типу `Integer` идентификатора перечисления.
====
