...
{jmix-docs}/data-access/entity-events.html#entity-changed-event[EntityChangedEvent^] listeners can be used to maintain data consistency and execute business logic in the current {jmix-docs}/data-access/transactions.html[transaction^] or after its completion.