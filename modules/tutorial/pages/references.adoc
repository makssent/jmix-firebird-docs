...
Как и в xref:simple-crud.adoc#run-app[предыдущей главе], таблицу для новой сущности придется описать самостоятельно.

Создайте файл `030-create-department.xml` - в разделе *Data Stores* -> *Main Data Store* -> *Liquibase: changelog.xml* окна инструментов *Jmix*, либо в каталоге `src/main/resources/com/company/onboarding/liquibase/changelog` в дереве проекта. Он должен создавать таблицу `DEPARTMENT` с уникальным ограничением на столбец `NAME`, индексом по столбцу внешнего ключа и внешним ключом на таблицу `USER_`. Ниже приведён готовый пример конфигурации, который можно использовать если возникают сложности с самостоятельным написанием.
...
<databaseChangeLog
    ...
    <changeSet id="1" author="onboarding">
        <createTable tableName="DEPARTMENT">
            <column name="ID" type="${uuid.type}">
                <constraints primaryKey="true" primaryKeyName="PK_DEPARTMENT" nullable="false"/>
            </column>
            ...
            <column name="HR_MANAGER_ID" type="${uuid.type}"/>
        </createTable>
    </changeSet>

    <changeSet id="2" author="onboarding">
        <addUniqueConstraint tableName="DEPARTMENT" columnNames="NAME"
                             constraintName="IDX_DEPARTMENT_UNQ_NAME"/>
    </changeSet>
    ...
    <changeSet id="4" author="onboarding">
        <addForeignKeyConstraint baseColumnNames="HR_MANAGER_ID"
                                 baseTableName="DEPARTMENT"
                                 constraintName="FK_DEPARTMENT_ON_HR_MANAGER"
...
Четыре changeset: таблица, уникальное ограничение, индекс по столбцу внешнего ключа и сам внешний ключ. Так Liquibase применит изменения по частям, и если одно из них не пройдет, остальные останутся в силе.

Имена ограничения и индекса взяты из аннотаций сущности - это те самые `IDX_DEPARTMENT_UNQ_NAME` и `IDX_DEPARTMENT_HR_MANAGER`, которые вы задали в дизайнере. Переименовывать их не стоит: по имени ограничения Jmix находит сообщение, которым заменяет системную ошибку базы данных. Как это работает, показано в разделе xref:references.adoc#change-unique-constraint-message[Изменение уникального сообщения о нарушении ограничений].

Столбец `HR_MANAGER_ID` объявлен без ограничения `nullable`, поскольку ссылка `hrManager` необязательная. Тип столбца тот же `${uuid.type}`, что и у первичного ключа: внешний ключ должен совпадать по типу с колонкой, на которую ссылается.
====

Теперь запустите приложение из консоли:
...
Найдите `departmentsDataGrid` в панели структуры *Jmix UI*. Компонент будет выбран в панель предварительного просмотра, в редакторе XML и в панели инспектора *Jmix UI* в правом нижнем углу:
...
* Studio помогает создавать ссылочные атрибуты и генерирует {jmix-docs}/data-model/db-migration.html[Liquibase changelog^] с внешним ключом и индексом.

* Чтобы показать ссылочный атрибут на экране списка или деталей, он должен быть включен в {jmix-docs}/data-access/fetching.html#fetch-plan[фетч-план^] экрана.

* {jmix-docs}/data-model/entities.html#instance-name[Имя экземпляра^] используется для отображения ссылки в пользовательском интерфейсе.

* Компонент выбора сущности ({jmix-docs}/flow-ui/vc/components/entityPicker.html[EntityPicker^]) используется по умолчанию для выбора связанной сущности в сгенерированном экране редактирования.

* {jmix-docs}/data-model/entities.html#uniqueness[Уникальность атрибутов сущностей^] поддерживается на уровне базы данных путем определения ограничений.

* Сообщение о {jmix-docs}/flow-ui/exception-handlers.html#unique-constraint-violation-handler[нарушении ограничений уникальности^] может быть легко кастомизировано.

* Заголовки и сообщения, сгенерированные Studio, хранятся в {jmix-docs}/localization/message-bundles.html[пакете сообщений^] приложения.

* Механизм Studio {jmix-docs}/studio/hot-deploy.html[hot deploys^] изменяет экраны и сообщения в запущенном приложение, что избавляет от перезапуска приложения при разработке пользовательского интерфейса. Hot deploy не работает для классов сущностей.