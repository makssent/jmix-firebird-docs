...
[WARNING]
====
Скрипт Liquibase для таблицы `DEPARTMENT` с уникальным ограничением на столбец `NAME`, внешним ключом и индексом на `HR_MANAGER_ID` Studio не сгенерирует - его нужно написать вручную, как показано в разделе xref:simple-crud.adoc#run-app[Запуск приложения] предыдущей главы.
====
