...
Как и в xref:simple-crud.adoc#run-app[предыдущей главе], таблицу для новой сущности придется описать самостоятельно: для поддерживаемых СУБД Studio сгенерировала бы changelog перед запуском, а с Firebird автогенерация недоступна.
