...
Создайте файл `030-create-department.xml` в каталоге `src/main/resources/com/company/onboarding/liquibase/changelog`:
...
Здесь четыре changeset вместо одного: таблица, уникальное ограничение, индекс по столбцу внешнего ключа и сам внешний ключ. Так Liquibase применит изменения по частям, и если одно из них не пройдет, остальные останутся в силе.
...
В выводе появятся строки обо всех четырех изменениях:

[source]
----
Table DEPARTMENT created
Unique constraint added to DEPARTMENT(NAME)
Index IDX_DEPARTMENT_HR_MANAGER created
Foreign key constraint added to DEPARTMENT (HR_MANAGER_ID)
----
