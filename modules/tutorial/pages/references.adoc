...
As in the xref:simple-crud.adoc#run-app[previous chapter], you need to define the table for the new entity manually. Studio would generate the changelog before startup for a supported database, but automatic changelog generation is not available for Firebird.

Create the `030-create-department.xml` file using the *Data Stores* -> *Main Data Store* -> *Liquibase: changelog.xml* section of the *Jmix* tool window, or in the `src/main/resources/com/company/onboarding/liquibase/changelog` directory in the project tree. It should create the `DEPARTMENT` table with a unique constraint on the `NAME` column, an index on the foreign key column, and a foreign key to the `USER_` table.
...
The four changesets create the table, the unique constraint, the index on the foreign key column, and the foreign key itself. Liquibase applies the changes separately, so if one fails, the preceding changes remain applied.

The constraint and index names come from the entity annotations: `IDX_DEPARTMENT_UNQ_NAME` and `IDX_DEPARTMENT_HR_MANAGER` are the names you specified in the designer. Do not rename them. Jmix uses the constraint name to find a message that replaces the database system error. This is demonstrated in xref:references.adoc#change-unique-constraint-message[Changing Unique Constraint Violation Message].

The `HR_MANAGER_ID` column has no `nullable` constraint because the `hrManager` reference is optional. Its type is the same `${uuid.type}` used by the primary key because a foreign key must have the same type as the column it references.
====
...
* Studio helps to create reference attributes and generates {jmix-docs}/data-model/db-migration.html[Liquibase changelogs^] with a foreign key and an index.

* To show a reference attribute in an entity list or detail view, it should be included in the {jmix-docs}/data-access/fetching.html#fetch-plan[fetch plan^] of the view.

* The {jmix-docs}/data-model/entities.html#instance-name[instance name^] is used to show a reference in the UI.

* The {jmix-docs}/flow-ui/vc/components/entityPicker.html[EntityPicker^] component is used by default to select a reference in a generated detail view.

* The {jmix-docs}/data-model/entities.html#uniqueness[uniqueness of entity attributes^] is maintained on the database level by defining unique constraints.

* A {jmix-docs}/flow-ui/exception-handlers.html#unique-constraint-violation-handler[unique constraint violation^] message can be easily customized.

* Captions and messages generated by Studio are stored in the {jmix-docs}/localization/message-bundles.html[message bundle^] of the application.

* Studio {jmix-docs}/studio/hot-deploy.html[hot deploys^] changes in views and messages to the running application, which saves from restarting the application when developing UI. Changes in entities are not hot deployed.