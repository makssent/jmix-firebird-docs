...
In the *New Resource Role* dialog, enter `Employee` into the *Role name* field and select `UI` in the *Security scope* dropdown:
...
In the *New Role* dialog, enter `HR Manager` into the *Role name* field, set *Role code* to `hr-manager`, and select `UI` in the *Security scope* dropdown:
...
* A {jmix-docs}/security/resource-roles.html[resource role^] gives users permissions to open views and to work with particular entities.

* A {jmix-docs}/security/row-level-roles.html[row-level role^], in contrast, restricts user's ability to read particular entity instances for an entity permitted by a resource role.
