...
* {jmix-docs}/security/resource-roles.html[Ресурсная роль^] предоставляет пользователям разрешения на открытие экранов и работу с определенными объектами.

* {jmix-docs}/security/row-level-roles.html[Роль уровня строки^], напротив, ограничивает способность пользователя просматривать определенные экземпляры для сущности, разрешенной ресурсной ролью.
