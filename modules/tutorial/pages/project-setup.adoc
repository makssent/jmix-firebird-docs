...
Open `build.gradle`. In the `dependencies` block, add the add-on and replace the HSQLDB dependency with the Firebird driver:
...
dependencies {
    ...
    // Replaced: use the Firebird JDBC driver instead of HSQLDB
...
The order is important. The add-on changelog declares Liquibase properties such as `${uuid.type}`, whose value is `varchar(36)` because Firebird has no dedicated UUID type. A declared property is visible only to scripts included below it, so the add-on changelog must come first.
