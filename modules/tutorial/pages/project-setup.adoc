...
Откройте `build.gradle` и в блоке `dependencies` подключите аддон, а зависимость HSQLDB замените на драйвер Firebird:
...
dependencies {
    ...
    // Заменено: вместо HSQLDB подключается JDBC-драйвер Firebird
...
Первое - главный changelog `com/company/onboarding/liquibase/changelog.xml`. Добавьте в него включение файла аддона, причем перед включениями модулей Jmix:
...
Порядок здесь важен. Файл аддона объявляет свойства Liquibase - например, `${uuid.type}` со значением `varchar(36)`, потому что отдельного типа UUID в Firebird нет. Объявленное свойство видно только тем скриптам, которые идут ниже по тексту, поэтому включение аддона должно стоять раньше остальных.

Второе - файл `com/company/onboarding/liquibase/changelog/010-init-user.xml`, где создается учетная запись администратора. В шаблоне проекта эти вставки ограничены списком СУБД, и Firebird в него не входит. Допишите его в атрибут `dbms` у обоих changeset-ов:
...
[NOTE]
====
Сборка не работает на JDK старше 21: запуск прервется ошибкой `Unsupported class file major version`, поскольку Gradle, которым собирается проект, такие версии пока не поддерживает.

Если основная JDK в системе новее, укажите подходящую явно:
